...
const int pinShtdE = 10;                                          // Pino de Entrada de sinal de shutdown Externo
const int pinBrk = 13;                                            // Pino de entrada do sinal de Freio
...
boolean estadoBrk = 0;              // estado do Freio
...
void loop()
{
  ...
  {
    estadoBrk = digitalRead(pinBrk);                // Lê se o freio esta acionado ou não
    digitalWrite(pinSeatswitch, HIGH);              // fecha a seatswitch
    estadoRTD = digitalRead(pinRTD);                // Lê estado do botão RTD
    if(estadoRTD == HIGH)                           // se o pino RTD estiver 
    {
      ...
      while(1)                                      // loop infinito onde vai pegar os valores dos sensores do acelerador
      {
       ...
       shtdE = digitalRead(pinShtdE);
       freio = analogRead(pinFreio);
       //========================================================================================================================================================
       // PLAUSIBILIDADE
       
       if(shtdE)                                      // Se shtdE for LOW (1) vai entrar no if
       {
        shtd = 1;
        shtdout = 1;
        digitalWrite(pinShtd, shtd);
